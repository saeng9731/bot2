...
def setup_sql_mod(dbapi_connection, connection_record):
    """SQL 모드 설정 (open_api.py에서 분리 - Linux 호환)"""
...
def create_training_engine(db_name):
    # pd.read_sql 과 호환되도록 SQLAlchemy 엔진 사용 (pymysql DictCursor는
    # pandas에서 컬럼명이 값으로 들어가는 버그가 있어 사용하지 않음)
    from sqlalchemy import create_engine
    return create_engine(
        "mysql+pymysql://" + cf.db_id + ":" + cf.db_passwd + "@" + cf.db_ip + ":" + cf.db_port + "/" + db_name,
        encoding='utf-8'
    )
...
def ai_filter(ai_filter_num, engine, until=datetime.datetime.today()):
    if ai_filter_num == 1:
        ...
        try:
            ...
            print("AI 필터 종료 (realtime_daily_buy_list 테이블 없음)")
            sys.exit(1)
        ...
        for code_name, in buy_list:
            ...
            # (2026-08-07) daily_craw 에 테이블이 없는 종목(신규/폐지/미수집)이 있으면
            # pd.read_sql 이 예외를 던져 시뮬레이터 전체가 중단됨. 해당 종목은 제외 처리.
            try:
                df = pd.read_sql(sql, tr_engine)
            except Exception as e:
                print(f"daily_craw 에 {code_name} 테이블이 없어 realtime_daily_buy_list 에서 제외 ({e})")
                filtered_list.append(code_name)
                continue

            # 데이터가 1000개(1000일 or 1000분)가 넘지 않으면 예측도가 떨어지기 때문에 필터링
            # (2026-08-13 수정) 윈도우 덤프 복원으로 daily_craw 전체 역사(1000일+) 확보 → 기준 1000일 복원
            if len(df) < 1000:
                filtered_list.append(code_name)
...
if __name__ == '__main__':
    # 모의투자, 실전투자 일때만 들어오는 함수
    # 인자가 없으면 기본값 사용 (JackBot1_imi1, simul_num=1)
    if len(sys.argv) >= 3:
        filter_by_ai(sys.argv[1], sys.argv[2])
    else:
        print("인자가 없어 기본값으로 실행합니다: JackBot1_imi1 1")
        filter_by_ai('JackBot1_imi1', '1')
