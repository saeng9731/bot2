...
class KINDCrawler:
    ...
    def check_version(self):
        ...
        if br_ver[:2] != dr_ver[:2]:
            print(
                ...
                'chromedriver 파일을 다운 받아 설치해 주시기 바랍니다.'
                '\n\n'
    ...
    def insert_to(self, file_name, table_name):
        ...
        # 크롬이 죽어 다운로드가 안 되는 경우를 대비해 120초 타임아웃 추가
        wait_sec = 0
        while not list(self.download_path.glob(self.FNAME_PATTERN)):
            if wait_sec > 120:
                raise TimeoutException(f"[안정화] {file_name} 다운로드 120초 대기 초과 - 크롬이 응답하지 않습니다.")
            sleep(1)
            wait_sec += 1
    ...
    # 로컬 엑셀 파일로 DB 반영 (Akamai 403 우회용, 해당 기간 replace)
    def insert_to_local(self, file_name, table_name):
        p = self.download_path / file_name
        print(f"[KIND-로컬] {file_name} → {table_name} 반영 시작")
        ...
        if len(df):
            ...
            # 해당 기간 기존 데이터를 먼저 제거한 뒤 append (중복 방지 + 해제일 갱신)
            min_d = df['post_date'].min()
            max_d = df['post_date'].max()
            if self.is_simul_table_exist(table_name):
                self.db_engine.execute(
                    "DELETE FROM {} WHERE post_date >= '{}' AND post_date <= '{}'".format(
                        table_name, min_d, max_d))
        ...
        print(f"[KIND-로컬] {file_name} → {table_name} 반영 완료 ({len(df)}행)")
    ...
    def _crawl_and_insert_once(self, file_name, table_name):
    ...
    # crawling하고, db에 넣는 함수 (크롬이 죽으면 재시작 후 이어서 진행)
    def crawl_and_insert(self, file_name, table_name, max_retry=3):
        for attempt in range(max_retry):
            try:
                self._crawl_and_insert_once(file_name, table_name)
                return
            except Exception as e:
                print(f"[안정화] {file_name} 크롤링 중 오류 발생 (시도 {attempt + 1}/{max_retry}): {e}")
                # 크롬이 죽었을 수 있으니 잔여 프로세스 정리 후 재시작
                self._cleanup_chrome()
                if not self.create_driver():
                    print(f"[안정화] 크롬 재시작 실패 → {file_name} 처리 건너뜀")
                    return
                # KIND 메인 페이지로 재접속 후 다음 시도
                try:
                    ...
                    pass
                sleep(3)
        print(f"[안정화] {file_name} 크롤링 {max_retry}회 실패 → 건너뜀")


    # ── 크롬 드라이버 생성 (실패 시 자동 재시도) ──
    def create_driver(self, max_retry=3):
        for attempt in range(max_retry):
            try:
                options = webdriver.ChromeOptions()
                # Linux 서버(오라클 클라우드)에서는 headless 모드로 실행
                if cf.IS_LINUX:
                ...
                # ARM 환경에서는 playwright chromium 바이너리를 브라우저로 사용
                if getattr(self, 'arm_browser_path', None):
                    options.binary_location = self.arm_browser_path

                self.driver = webdriver.Chrome(service=Service(executable_path=path), options=options)
                self.driver.implicitly_wait(10)
                self.actions = ActionChains(self.driver)
                self.check_version()
                print(f"크롬 드라이버 생성 성공 (시도 {attempt + 1}/{max_retry})")
                return True
            except Exception as e:
                print(f"[안정화] 크롬 드라이버 생성 실패 (시도 {attempt + 1}/{max_retry}): {e}")
                self._cleanup_chrome()
                sleep(5)
        return False
    ...
    def _cleanup_chrome(self):
        if cf.IS_LINUX:
            os.system('pkill -f ms-playwright/chromium 2>/dev/null')
            os.system('pkill -f chromedriver 2>/dev/null')
            sleep(2)

    # 크롤링 시작하는 함수
    def craw(self):
        # ---- Akamai 403 우회: 로컬에서 받아둔 엑셀이 있으면 셀레니움 없이 DB 반영 ----
        # (서버 IP는 kind.krx.co.kr 접속이 403이므로, 로컬(집)에서 받은 파일을 사용)
        local_map = {
            'invest_caution.xls': 'stock_invest_caution',
            'invest_warning.xls': 'stock_invest_warning',
            'invest_danger.xls': 'stock_invest_danger',
        }
        if all((self.download_path / fn).exists() for fn in local_map):
            print("[KIND-로컬] 로컬 엑셀 감지 → 셀레니움 없이 DB 반영 (Akamai 403 우회)")
            for fn, tb in local_map.items():
                try:
                    self.insert_to_local(fn, tb)
                except Exception as e:
                    print(f"[KIND-로컬] {fn} 처리 실패: {e}")
            return
        ...
        # 드라이버 생성 (실패 시 자동 재시도 포함)
        if not self.create_driver():
            print("[안정화] 크롬 드라이버 생성 실패로 크롤링을 중단합니다.")
            return
        ...
        self.driver.get('http://kind.krx.co.kr/investwarn/investattentwarnrisky.do?method=investattentwarnriskyMain')
        self.dialog_block_wait() # 대기
        self.take_snapshot("b-candi.png") # 디버깅용 캡처
    ...
    def chrome_driver_update(self):
        ...
        # ── Linux ARM(aarch64) 환경에서는 snap chromium의 chromedriver를 사용 ──
        # 공식 chromedriver 배포본에 linux-arm64가 없어서 chromedriver-autoinstaller가
        # x86_64 드라이버를 받아 "Exec format error"가 발생한다.
        # ARM 서버에서는 snap으로 설치한 chromium.chromedriver를 그대로 사용한다.
        # (브라우저는 playwright chromium을 사용 - snap chromium 브라우저는 서버에서 core dump 발생)
        if cf.IS_LINUX and platform.machine() in ('aarch64', 'arm64'):
            snap_cd = '/snap/bin/chromium.chromedriver'
            if os.path.exists(snap_cd):
                print(f"ARM 환경 감지: snap chromium chromedriver 사용 → {snap_cd}")
                # playwright로 설치한 chromium 브라우저 경로 탐색
                import glob
                pw_chrome = glob.glob(os.path.expanduser('~/.cache/ms-playwright/chromium-*/chrome-linux/chrome'))
                if pw_chrome:
                    self.arm_browser_path = pw_chrome[-1]
                    print(f"ARM 환경 감지: playwright chromium 브라우저 사용 → {self.arm_browser_path}")
                else:
                    self.arm_browser_path = None
                    print("[경고] playwright chromium이 없습니다. `python -m playwright install chromium` 실행이 필요합니다.")
                return snap_cd
            else:
                print("[경고] /snap/bin/chromium.chromedriver 가 없습니다. snap chromium 설치가 필요합니다.")
