...
class Trader:
    ...
    def variable_setting(self):
        ...
        ################ 모의, 실전 ####################
        # 장시작 시간 설정
        self.market_start_time = datetime.time(9, 0, 0)
        # 장마감 시간 설정
        self.market_end_time = datetime.time(15, 30, 0)
        # 매수를 몇 시 까지 할지 설정. (시, 분, 초)
        self.buy_end_time = datetime.time(9, 6, 0)
    ...
    def market_time_check(self):
        ...
        self.current_time = datetime.datetime.now()
        # current_time(datetime.datetime) 과 market_start_time(datetime.time) 비교를 위해 .time() 변환
        if self.current_time.time() > self.market_start_time and self.current_time.time() < self.market_end_time:
    ...
    def buy_time_check(self):
        ...
        self.current_time = datetime.datetime.now()
        if self.current_time.time() < self.buy_end_time:
            return True
        else:
