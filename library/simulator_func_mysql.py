...
def escape_percentage(conn, clauseelement, multiparams, params):
    """execute로 실행한 sql문이 들어왔을 때 %를 %%로 replace (open_api.py에서 분리 - Linux 호환)"""
...
class simulator_func_mysql:
    ...
    def variable_setting(self):
        ...
        if self.simul_num in (1, 4, 5, 7, 8, 9, 10, 11, 12, 13, 14, 16, 17, 18):
            ...
            # 실시간 조건 매수
                ...
                # (2026-08-06 부터 금융 데이터 수집이 정상화되었으므로 20260807 로 변경)
                self.simul_start_date = "20260807"
    ...
    def db_to_realtime_daily_buy_list(self, date_rows_today, date_rows_yesterday, i):
        ...
        if len(realtime_daily_buy_list) > 0:
            ...
            # 2026-08-07: 스팩/신규상장 종목은 코드에 영문이 포함(예: 0030R0)되어
            #             int() 변환 시 ValueError 발생. 문자열 그대로 zfill(6)로 처리.
        ...
        # len_df_realtime_daily_buy_list에 다가 0을 넣는다.
            ...
            if inspect(self.engine_simulator).has_table("realtime_daily_buy_list"):
    ...
    def get_date_for_simul(self):
        try:
            # (2026-08-13 수정) 테이블명 대소문자: gs글로벌 → GS글로벌 (리눅스는 대소문자 구분)
    ...
    def arrange_jango_data(self):
        if inspect(self.engine_simulator).has_table('jango_data'):
