...
import numpy
import pathlib
from library.kiwoom_api import *
import os
import time
...
class collector_api():
    ...
    def code_update_check(self):
        ...
        if self.open_api.sf.use_ai:
            # Linux 서버(오라클 클라우드)에서는 shell script 실행, Windows에서는 bat 파일 실행
            if cf.IS_LINUX:
                path = pathlib.Path(__file__).parent.parent.absolute() / 'sh' / 'ai_filter.sh'
                os.system(f"bash {path} {self.open_api.db_name} {self.open_api.simul_num}")
            else:
    ...
    def db_to_min_craw(self):
        ...
        for i in range(num):
            ...
            try:
                ...
                # rate limit(429) 초과 시 콜렉터 전체를 죽이지 않고 대기 후 다음 종목으로 넘어간다.
                # (check_min_crawler를 갱신하지 않으므로 다음 실행에서 이 종목부터 재시도됨)
                logger.warning(f"[WARN] ka10080 rate limit 초과 ({code_name}). 60초 대기 후 계속합니다.")
                time.sleep(60)
                continue
    ...
    def _stock_to_sql(self, origin_df, type):
        ...
        for KIND_info in origin_df.itertuples():
            # KIND 크롤링에서 가져온 code_name이 있으면 API 호출 없이 그대로 사용 (속도 대폭 향상)
            # ka10001 REST API는 초당 1회만 호출 가능하므로, KIND 데이터에 이미 종목명이 있으면 API 호출을 생략
            kind_name = str(getattr(KIND_info, 'code_name', '')).strip()
            if kind_name:
                name_list.append(kind_name)
                continue

            # KIND에 종목명이 없을 때만 키움 API로 조회 (rate limit 1초 대기 포함)
        ...
        if check_daily_crawler == '4':
            ...
            for row in dbl_dates:
                ...
                try:
                    ...
                    continue
                if inspect(self.open_api.engine_daily_buy_list).has_table(row.tname):
    ...
    def _create_stock_info(self):
        ...
        # ---------- REST 대응 ----------
        # 원본(Windows OCX)은 종목마다 GetMasterConstruction/GetMasterStockState/GetMasterStockInfo로
        # 감리구분/증거금/시장구분/종목분류를 채웠지만, REST ka10001 응답에는 해당 필드가 없다.
        # 따라서 아래와 같이 채운다.
        #   - stock_market : stock_kospi/kosdaq/konex 테이블에서 정확히 파생 (DB 조회, API 호출 없음)
        #   - remarks      : KIND 크롤링 결과(stock_managing/stock_insincerity)로 관리종목/불성실공시법인 마킹
        #   - audit        : ka10001에 감리구분이 없어 기본값 '정상'
        #   - category0    : ka10001에 종목분류가 없어 기본값 '우량기업'
        #   - margin       : ka10001에 증거금비율이 없어 기본값 40 (알고리즘 조건: 40 이하 매수)
        codes = stock_info_data['code']
        n = len(codes)

        # 시장구분: 종목별 시장 테이블에서 파생
        market_map = {}
        try:
            for tbl, market in (('stock_kospi', '거래소'), ('stock_kosdaq', '코스닥'), ('stock_konex', '코넥스')):
                for r in self.open_api.engine_daily_buy_list.execute(f"SELECT code FROM {tbl}").fetchall():
                    market_map[str(r[0])] = market
        except Exception as e:
            logger.warning(f"[WARN] 시장구분 테이블 조회 실패: {e}")

        # 관리종목 / 불성실공시법인 (KIND 크롤링 결과 테이블)
        managing_codes = set()
        insincerity_codes = set()
        try:
            for r in self.open_api.engine_daily_buy_list.execute("SELECT code FROM stock_managing").fetchall():
                managing_codes.add(str(r[0]))
        except Exception:
            pass
        try:
            for r in self.open_api.engine_daily_buy_list.execute("SELECT code FROM stock_insincerity").fetchall():
                insincerity_codes.add(str(r[0]))
        except Exception:
            pass

        stock_info_data['audit'] = ['정상'] * n
        stock_info_data['margin'] = [40] * n
        stock_info_data['thema_code'] = [None] * n
        stock_info_data['thema_name'] = [None] * n
        stock_info_data['category0'] = ['우량기업'] * n
        stock_info_data['market_class0'] = [None] * n
        stock_info_data['market_class1'] = [None] * n
        stock_info_data['category1'] = [None] * n
        stock_info_data['stock_market'] = []
        stock_info_data['remarks'] = []

        for c in codes:
            market = market_map.get(str(c), '')
            remarks = []
            if str(c) in managing_codes:
                remarks.append('관리종목')
            if str(c) in insincerity_codes:
                remarks.append('불성실공시법인')
            stock_info_data['stock_market'].append(market)
            stock_info_data['remarks'].append('|'.join(remarks))
    ...
    def _create_stock_finance(self):
        ...
        strformat = "%Y%m%d"
        extract_from = datetime.date.today() - datetime.timedelta(days=10)
        extract_from = extract_from.strftime(strformat)

        if inspect(self.open_api.engine_daily_buy_list).has_table(table_name):
            try:
                # 기존 테이블이 올바른 구조(code 컬럼)인지 확인
                self.open_api.engine_daily_buy_list.execute(f"""
                    SELECT date, code FROM {table_name} WHERE date >= {extract_from} LIMIT 1
                """).fetchall()
                # 구조가 맞으면 10일치 전체 조회로 교체 (set으로 변환해 빠른 중복체크)
                rows = self.open_api.engine_daily_buy_list.execute(f"""
                    SELECT date, code FROM {table_name} WHERE date >= {extract_from}
                """).fetchall()
                existing_data = set((str(r[0]), str(r[1])) for r in rows)
            except Exception:
                # 기존 stock_finance 테이블이 잘못된 구조일 경우 삭제 후 새로 생성
                logger.warning(f"[WARN] 기존 {table_name} 테이블 구조가 올바르지 않아 삭제 후 재생성합니다.")
                self.open_api.engine_daily_buy_list.execute(f"DROP TABLE IF EXISTS {table_name}")
                existing_data = set()
        ...
        skipped_count = 0
        empty_count = 0
        total = len(stock_codes_list)
        for i, c in enumerate(stock_codes_list):  # index를 얻기위해 enumerate() 사용
            # 자정(00:00)이 지나면 날짜가 바뀌므로 매 루프마다 오늘 날짜로 갱신한다.
            today = datetime.date.today().strftime(strformat)
            ...
            if not fin_data:
                # 데이터가 없으면 저장하지 않고 재시도 대상으로 남긴다.
                # (빈 값으로 테이블을 오염시키지 않고, 다음 실행 때 다시 시도)
                empty_count += 1
                if empty_count % 100 == 0:
                    logger.warning(f"[WARN] 금융 데이터 없음 {empty_count}건 (다음 실행 때 재시도)")
                continue

            data['date'].append(today)  # 날짜 칼럼 추가
            # 종목코드는 응답에 없어도 루프 변수(c)로 항상 기록 → 중복체크가 동작하도록 보장
            code_value = fin_data.get('종목코드')
            data['code'].append(code_value if code_value else c)

            for k, v in fin_data.items():
                if k not in column_names or k == '종목코드':
                    continue  # 테이블 컬럼에 없는 키는 버린다 (스키마 일관성 유지)
                if v == '':  # 비어있는 문자열일 경우 None으로 대체
                    converted = None
                elif v and (k in signed_columns):  # +, -기호가 추세를 나타내는 칼럼은 절대값으로
                    try:
                        converted = abs(float(v))
                    except (TypeError, ValueError):
                        converted = None
                else:
                    converted = v
                data[column_names[k]].append(converted)
            logger.debug(f'{c} 금융 데이터 요청중...')

            item_count += 1

            if item_count >= chunk_size or i == total - 1:  # chunk_size 단위로 저장 83개는 max_api_call 999에 최적화
            ...
            if (i + 1) % 100 == 0:
                logger.info(f'[금융] {i + 1}/{total} 처리 중 (건너뜀 {skipped_count}, 데이터없음 {empty_count})')
