...
# Linux(오라클 클라우드)에서는 PyQt5를 사용하지 않음 (Windows 전용)
...
class daily_craw_config():
    ...
    def get_item_insincerity(self):
        ...
        self.code_df_insincerity = pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=05', header=0, encoding='euc-kr')[0]
    ...
    def get_item_managing(self):
        ...
        self.code_df_managing = pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=01', header=0, encoding='euc-kr')[0]  # 종목코드가 6자리이기 때문에 6자리를 맞춰주기 위해 설정해줌

        # 6자리 만들고 앞에 0을 붙인다.strPath --> str(unicode(strPath))
        # 종목코드가 숫자형으로 파싱될 수 있으므로 문자열로 변환 후 zfill 적용
    ...
    def get_item_konex(self):
        ...
        self.code_df_konex = pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=13&marketType=konexMkt',header=0, encoding='euc-kr')[0]  # 종목코드가 6자리이기 때문에 6자리를 맞춰주기 위해 설정해줌
    ...
    def get_item_kospi(self):
        ...
        self.code_df_kospi = \
        pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=13&marketType=stockMkt',header=0, encoding='euc-kr')[0]  # 종목코드가 6자리이기 때문에 6자리를 맞춰주기 위해 설정해줌
    ...
    def get_item_kosdaq(self):
        ...
        self.code_df_kosdaq = pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=13&marketType=kosdaqMkt',header=0, encoding='euc-kr')[0]  # 종목코드가 6자리이기 때문에 6자리를 맞춰주기 위해 설정해줌
    ...
    def get_item(self):
        # print("get_item!!")
        self.code_df = pd.read_html('http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=13', header=0, encoding='euc-kr')[0]  # 종목코드가 6자리이기 때문에 6자리를 맞춰주기 위해 설정해줌
